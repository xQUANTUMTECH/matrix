export class ActionManager {
    ...
    async _executeAction(actionLabel, actionFn, timeout = 10) {
        ...
        try {
            ...
            // Log avvio azione su DB
            try {
                const { db } = await import('../db/db.js');
                if (db.isConnected()) await db.saveAction(this.agent.name, actionLabel, 'started');
            } catch (_) {}
            ...
            // Log completamento azione su DB
            try {
                const { db } = await import('../db/db.js');
                if (db.isConnected()) await db.saveAction(this.agent.name, actionLabel, timedout ? 'timed_out' : interrupted ? 'interrupted' : 'completed');
            } catch (_) {}
            ...
            // PathStopped è un'interruzione voluta dal pathfinder (ostacolo, chunk reload, ecc.)
            // Non è un errore logico: trattarlo come interruzione pulita evita il loop di retry
            const isPathStopped = err?.name === 'PathStopped' || err?.message?.includes('PathStopped') || String(err).includes('PathStopped');

            if (isPathStopped) {
                console.warn('[ActionManager] PathStopped caught — treating as clean interruption, not error.');
                await this.stop();
                this.agent.clearBotLogs();
                this.agent.bot.emit('idle');
                try {
                    const { db } = await import('../db/db.js');
                    if (db.isConnected()) {
                        await db.saveAction(this.agent.name, actionLabel, 'path_blocked');
                        if (actionLabel) {
                            const lesson = `Il percorso durante "${actionLabel}" era bloccato — prova un percorso diverso o scegli un obiettivo alternativo vicino.`;
                            await db.saveLesson(this.agent.name, lesson, actionLabel);
                        }
                    }
                } catch (_) {}
                return { success: false, message: 'Il percorso è stato interrotto (ostacolo o chunk aggiornato). Azione annullata.', interrupted: true, timedout: false };
            }
            ...
            try {
                const { db } = await import('../db/db.js');
                if (db.isConnected()) await db.saveAction(this.agent.name, actionLabel, 'failed', String(err));
            } catch (_) {}
            err = err.toString();
