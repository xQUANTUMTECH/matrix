...
// Normalizza virgolette singole → doppie dentro le parentesi dei comandi
// Es: !startConversation('Foo', 'bar') → !startConversation("Foo", "bar")
function normalizeCmdQuotes(message) {
    return message.replace(/!([\w]+)\(([^)]*)\)/g, (match, cmd, args) => {
        const normalized = args.replace(/'([^']*)'/g, '"$1"');
        return `!${cmd}(${normalized})`;
    });
}

export function containsCommand(message) {
    const commandMatch = normalizeCmdQuotes(message).match(commandRegex);
    if (commandMatch)
...
export function parseCommandMessage(message) {
    message = normalizeCmdQuotes(message);
