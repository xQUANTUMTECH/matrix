...
export const queryList = [
    {
        ...
        perform: function (agent) {
            ...
            if (!bot.entity || !bot.entity.position)
                return 'Stats not available yet, bot is still spawning.';
            let res = 'STATS';
    ...
    {
        name: '!remember',
        description: 'Save an important fact, observation, or memory to your long-term database. Use this to remember things about other agents, locations, events, or discoveries. Example: !remember("Redstone mi ha dato del legno vicino alla foresta a nord")',
        params: {
            'content': { type: 'string', description: 'The fact or memory to save (max 500 characters).' }
        },
        perform: async function (agent, content) {
            try {
                const { db } = await import('../../db/db.js');
                if (!db.isConnected()) return 'Memoria non disponibile al momento.';
                await db.saveNote(agent.name, content);
                return `Ricordato: "${content}"`;
            } catch (e) {
                return `Errore nel salvare la memoria: ${e.message}`;
            }
        }
    },
    {
        name: '!recall',
        description: 'Search your long-term memory for facts related to a query. Use this to remember things about other agents, past events, or locations. Example: !recall("Redstone") or !recall("legno")',
        params: {
            'query': { type: 'string', description: 'The topic or keyword to search in your memories.' }
        },
        perform: async function (agent, query) {
            try {
                const { db } = await import('../../db/db.js');
                if (!db.isConnected()) return 'Memoria non disponibile al momento.';
                const notes = await db.searchNotes(agent.name, query, 8);
                if (notes.length === 0) return `Nessun ricordo trovato su "${query}".`;
                const lines = notes.map(n => `- ${n.content}`).join('\n');
                return `Ricordi su "${query}":\n${lines}`;
            } catch (e) {
                return `Errore nella ricerca: ${e.message}`;
            }
        }
    },
    {
        name: '!searchItem',
        description: 'Search the Minecraft item database by name or minecraft ID. Returns matching items with their exact minecraft IDs. Use this when you are unsure of the correct item name for crafting, placing, or collecting.',
        params: {
            'query': { type: 'string', description: 'The item name or partial name to search for (e.g. "sword", "oak", "iron").' }
        },
        perform: async function (agent, query) {
            try {
                const { db } = await import('../../db/db.js');
                if (!db.isConnected()) {
                    return 'Item database not available. Try using the item name directly.';
                }
                const results = await db.searchItems(query, 10);
                if (results.length === 0) {
                    return `No items found matching "${query}". Try a different search term.`;
                }
                const lines = results.map(r => `- ${r.display_name} → ${r.minecraft_id}`).join('\n');
