    ...
    },
    {
        ...
        perform: async function (agent, player_name, message) {
            if (!convoManager.isOtherAgent(player_name)) {
                const online = convoManager.getInGameAgents().filter(n => n !== agent.name);
                return `${player_name} non è un agente noto. Agenti online ora: ${online.join(', ') || 'nessuno'}.`;
            }
            if (!convoManager.otherAgentInGame(player_name)) {
                const online = convoManager.getInGameAgents().filter(n => n !== agent.name);
                return `${player_name} è registrato ma non ancora in gioco (si sta ancora connettendo). Riprova tra qualche secondo oppure parla con chi è già online: ${online.join(', ') || 'nessuno al momento'}.`;
            }
            if (convoManager.inConversation() && !convoManager.inConversation(player_name))
                convoManager.forceEndCurrentConversation();
            else if (convoManager.inConversation(player_name))
                agent.history.add('system', 'Sei già in conversazione con ' + player_name + '. Non usare questo comando, scrivi direttamente.');
