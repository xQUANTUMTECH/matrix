...
const modes_list = [
    ...
    },
    {
        ...
        interrupts: ['action:moveAway', 'action:goToPosition', 'action:goToNearestBlock'],
        on: true,
        active: false,
        prev_location: null,
        distance: 1,
        ...
        update: async function (agent) {
            ...
            const max_stuck_time = cur_dig_block?.name === 'obsidian' ? this.max_stuck_time * 4 : this.max_stuck_time;
            if (this.stuck_time > max_stuck_time) {
                ...
                execute(this, agent, async () => {
                    ...
                    // 1. Prova a rompere i blocchi davanti (altezza 0 e 1) e sopra
                    const front  = bot.blockAt(pos.offset(Math.round(dx), 0, Math.round(dz)));
                    const front2 = bot.blockAt(pos.offset(Math.round(dx), 1, Math.round(dz)));
                    const above  = bot.blockAt(pos.offset(0, 2, 0));
                    let dug = false;
                    for (const blk of [front, front2, above]) {
                        if (blk && blk.name !== 'air' && blk.name !== 'water' && blk.diggable) {
                            try { await bot.dig(blk, true); dug = true; } catch (_) {}
                        }
                    }

                    // 2. Se non è riuscito a scavare, piazza un blocco sotto di sé per salire
                    if (!dug) {
                        const PILLAR_BLOCKS = ['cobblestone', 'dirt', 'stone', 'oak_planks', 'gravel',
                                               'sand', 'netherrack', 'oak_log', 'spruce_planks',
                                               'birch_planks', 'acacia_planks', 'jungle_planks'];
                        const pillarItem = bot.inventory.items().find(i => PILLAR_BLOCKS.includes(i.name));
                        if (pillarItem) {
                            try {
                                const floorBlock = bot.blockAt(pos.offset(0, -1, 0));
                                if (floorBlock && floorBlock.name !== 'air') {
                                    await bot.equip(pillarItem, 'hand');
                                    // Salta e piazza il blocco sotto di sé mentre è in aria
                                    bot.setControlState('jump', true);
