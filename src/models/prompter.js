...
import { NarrativeLibrary } from '../agent/library/narrative_library.js';
...
export class Prompter {
    constructor(agent, profile) {
        ...
        // Inietta placeholder $NARRATIVE nel conversing — verrà risolto dinamicamente
        this.profile.conversing = '$NARRATIVE\n\n' + this.profile.conversing;
        ...
        this.narrative_library = new NarrativeLibrary(this.embedding_model);
    ...
    async initExamples() {
        try {
            ...
            await Promise.all([
                ...
                this.narrative_library.init(db)
            ]).catch(error => {
    ...
    async replaceStrings(prompt, messages, examples=null, to_summarize=[], last_goals=null) {
        if (prompt.includes('$NARRATIVE')) {
            const narrative = await this.narrative_library.getRelevant(messages, this.agent.name, 2);
            prompt = prompt.replaceAll('$NARRATIVE', narrative);
        }
    ...
    async promptConvo(messages) {
        ...
        for (let i = 0; i < 3; i++) { // try 3 times to avoid hallucinations
            ...
            // Nuovo messaggio arrivato durante la generazione: non scartare la risposta già pronta.
            // Il messaggio in coda verrà processato separatamente dal conversation manager.
            if (current_msg_time !== this.most_recent_msg_time) {
                console.log(`${this.agent.name} ha ricevuto un nuovo messaggio durante la generazione — invio comunque la risposta corrente e la coda gestirà il successivo.`);
            }
